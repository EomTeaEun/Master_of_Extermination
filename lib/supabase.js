...
export async function saveScore({ userId, username, killCount, moneyEarned, waveReached, survivalTime, gameMode, endedWith, score }) {
  ...
  // 기존 기록 조회 — 중복이 있을 수 있으므로 limit(1) + 최고 기록 기준 정렬
  const orderCol = gameMode === 'daysurvival' ? 'survival_time' : 'score';
  const ascending = gameMode === 'daysurvival'; // 하루살이: 짧은 시간이 좋음

  const { data: rows, error: selErr } = await supabase
    .from('scores')
    .select('id, score, survival_time')
    .eq('user_id', userId)
    .eq('game_mode', gameMode)
    .order(orderCol, { ascending })
    .limit(1);

  if (selErr) { console.error('[saveScore] SELECT 에러:', selErr); return { data: null, error: selErr }; }

  const existing = rows?.[0] ?? null;

  if (existing) {
    const isBetter = gameMode === 'daysurvival'
      ? (survivalTime ?? Infinity) < (existing.survival_time ?? Infinity)
      : newScore > (existing.score ?? 0);

    if (!isBetter) return { data: null, error: null };

    // 중복 행 정리 후 베스트 기록 업데이트
    await supabase.from('scores').delete()
      .eq('user_id', userId).eq('game_mode', gameMode).neq('id', existing.id);

    const { data, error } = await supabase
      .from('scores')
      .update({
        kill_count: killCount, money_earned: moneyEarned,
        wave_reached: waveReached, survival_time: survivalTime,
        ended_with: endedWith, score: newScore,
        created_at: new Date().toISOString(),
      })
      .eq('id', existing.id);

    if (error) console.error('[saveScore] UPDATE 에러:', error);
    return { data, error };
  }
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      kill_count: killCount, money_earned: moneyEarned,
      wave_reached: waveReached, survival_time: survivalTime,
      game_mode: gameMode, ended_with: endedWith, score: newScore,
    });

  if (error) console.error('[saveScore] INSERT 에러:', error);
  return { data, error };
}
